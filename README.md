...
The automated quality layer also includes a deterministic property-style `invariants-smoke`, bounded DAP/ANSI/VSIX and JSON/LSP fuzz smokes, a small mutation-testing harness, opt-in strict-warning and sanitizer CMake profiles, and a dedicated Linux fuzz workflow. See [`docs/QUALITY_GATES.md`](docs/QUALITY_GATES.md) for reproducible commands, covered invariants, bounded budgets, and the checks that intentionally remain periodic or human-led.
