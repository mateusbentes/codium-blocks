...
Every push and every pull request is checked by isolated GitHub Actions workflows for the three target operating systems. The workflows build the native application, run the portable CTest suite, validate the versioned LSP/DAP matrices, check JavaScript syntax, and execute the Extension Host smoke test. The optional Code::Blocks SDK and DebuggerGDB jobs are isolated in a separate Linux workflow. Separate package workflows also run on every push, manual dispatch, and a weekly schedule to build Linux, macOS, and Windows artifacts without publishing a release automatically.

The automated quality layer also includes a deterministic property-style `invariants-smoke`, bounded DAP/ANSI/VSIX and JSON/LSP fuzz smokes with a versioned DAP corpus, a small mutation-testing harness, opt-in strict-warning, sanitizer, and Linux GCC coverage CMake profiles, clang-tidy and CodeQL analysis, deterministic SPDX/dependency reports, validated package manifests, and isolated real-toolchain matrices. See [`docs/QUALITY_GATES.md`](docs/QUALITY_GATES.md) for reproducible commands, covered invariants, bounded budgets, and the checks that intentionally remain periodic or human-led.
...
- an optional `tests/real-lsp-smoke.mjs` matrix that exercises installed clangd, rust-analyzer, gopls, and pyright-langserver binaries in language-appropriate temporary workspaces without making them build dependencies; for gopls, the harness explicitly announces the temporary workspace folder, gives startup a short grace period, and allows a bounded retry while the asynchronous workspace view is created on slower runners. The pinned Windows Pyright wrapper receives a bounded two-second post-`didOpen` analysis grace period and a 30-second request timeout for its slower first workspace analysis. The ordinary platform workflows preserve bounded environment skips, while the isolated real-toolchain workflows set strict mode and fail on missing required servers or incomplete scenarios; the Windows gopls entry is explicitly optional only when that runner reports the documented no-workspace-view limitation;
...
- an Open VSX client with HTTPS validation, bounded catalog parsing, offline catalog cache, same-origin VSIX download, post-download SHA-256 verification, native selection/install flow, installed-extension update checks, and selected-update installation;
...
- non-shell task variable expansion for workspace, configuration, target, toolchain, file, and explicitly named environment variables;
...
- a native localization layer with complete English and Brazilian Portuguese catalogs covering menus, command palette, controls, dialogs, statuses, terminal, Debug, LSP, extensions, and accessibility labels;
- system-language detection, an explicit **View > Language** menu, environment override through `CODIUM_BLOCKS_LANGUAGE`, and user preference persistence outside the source tree;
- English fallback for incomplete catalogs, with product identity, GPL-3.0-only wording, SPDX identifiers, protocol names, paths, commands, and user-provided technical values intentionally kept invariant.

This is not full VS Code or Code::Blocks compatibility yet. The implementation is deliberately layered. The versioned real-server and DAP matrices are mandatory contract checks, while runtime execution remains conditional on installed external tools and adapter capabilities. The native registry supports bounded Open VSX search, same-origin and configured-registry policies, private-registry bearer authentication, SHA-256 and optional Ed25519 verification, transactional installation, compatibility reports, and explicit unattended-update policy. The native contribution boundary now includes SCM resources, custom-editor providers, and a bounded webview bridge that sanitizes executable markup before publication. The native installation layout and isolated CPack package workflows now exist, and package evidence runs on every push, but the artifacts remain unsigned, unnotarized, and unpublished until clean-environment testing and human release review are complete. The private DebuggerGDB provider is opt-in and requires an exact matching source, SDK, compiler, wxWidgets, architecture, language-standard, and build identity; it is not inferred from a plugin filename. The build-oriented workflow now has explicit user tasks/schemes, generator-aware discovery, artifact selection, Build-and-Run orchestration, and a documented non-shell variable contract. The editor provides Find, Replace, Go to Line, Go to File, symbol/location navigation, completion application, rich hover presentation, rename/code-action application, delimiter matching, basic indentation, and tab navigation. The debugger synchronizes the active frame with the editor, persists source and advanced breakpoint specifications, reports adapter rejection messages, parses initialize capabilities, discovers adapter-specific data IDs, and runs bounded real GDB/LLDB scenarios when the required adapter is installed. The native workbench now has a complete first-party English/Brazilian Portuguese catalog, but human review on real high-DPI displays and assistive technologies remains necessary for accessibility claims. The terminal has broader automated coverage for supplementary Unicode, emoji modifiers, ZWJ sequences, regional indicators, and bounded Sixel/Kitty payload metadata; it does not claim to rasterize every image protocol. A mandatory splash screen is intentionally not part of startup; it should be added only if real asynchronous initialization later requires progress feedback. See [`docs/TASKS.md`](docs/TASKS.md), [`docs/PACKAGING.md`](docs/PACKAGING.md), [`docs/LOCALIZATION.md`](docs/LOCALIZATION.md), [`docs/CODEBLOCKS_INTEGRATION.md`](docs/CODEBLOCKS_INTEGRATION.md), [`docs/TERMINAL.md`](docs/TERMINAL.md), [`docs/DEBUGGING.md`](docs/DEBUGGING.md), [`docs/EXTENSIONS_SECURITY.md`](docs/EXTENSIONS_SECURITY.md), [`docs/CONTRIBUTIONS.md`](docs/CONTRIBUTIONS.md), [`docs/UI_DESIGN.md`](docs/UI_DESIGN.md), and [`docs/PROBLEMS.md`](docs/PROBLEMS.md) for the current models.
...
Node.js is optional. The native application can start without it and launch the Extension Host only when a JavaScript or TypeScript extension is needed. Extension webviews are represented through a bounded, sanitized host contract; the core process does not embed Chromium or execute extension-provided browser scripts.
...
The standard Open VSX path uses the native URL client. `curl` is additionally required only when a deployment configures bearer-authenticated private registries; those requests are restricted to HTTPS.
...
The remaining 1.0 work is organized by user-visible outcomes. Automated gates now run on every push and cover the fixed real-server matrix, coverage-guided fuzzing, static/security evidence, native package evidence, and the bounded SCM, extension, webview, Unicode, and DAP contracts. A complete keyboard, screen-reader, font-scaling, high-DPI accessibility audit, clean-machine trust review, signing, notarization, and release provenance remain human-controlled because they cannot be established by these deterministic checks. Each workstream preserves the adapter-disabled portable path and documentation in English.
...
A separate process reduces the impact of extension failures, but it is not a security sandbox. An extension may still inherit the user's permissions for files, processes, and network access. Workspace trust, configured registry allowlists, bounded host/webview messages, resource limits, package validation, and explicit signed-download policies are implemented; production distribution still requires operator review of permissions, logging, key rotation, clean-machine behavior, and release provenance.
