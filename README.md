...
- an optional `tests/real-lsp-smoke.mjs` matrix that exercises installed clangd, rust-analyzer, gopls, and pyright-langserver binaries in language-appropriate temporary workspaces without making them build dependencies; the harness gives gopls a short startup grace period and allows a bounded retry while it creates its asynchronous workspace view on slower runners, an unavailable or non-startable external toolchain is reported as skipped, and protocol failures after startup remain test failures;
