...
1. It starts its own `wxApp` and hidden `wxFrame`, following the Code::Blocks application lifecycle boundary [1](https://svn.code.sf.net/p/codeblocks/code/trunk/src/src/app.cpp).
...
6. It calls `ProjectManager::LoadProject()` and enumerates real `cbProject` build targets through `GetBuildTargetsCount()` and `GetBuildTarget()` [2](https://svn.code.sf.net/p/codeblocks/code/trunk/src/sdk/projectmanager.cpp).
...
8. When requested, it creates a private staging directory containing only the matched Compiler and Debugger files from one plugin directory. It uses the SDK `ScanForPlugins()` path for that staging directory instead of loading two foreign modules through unrelated direct calls [3](https://svn.code.sf.net/p/codeblocks/code/trunk/src/sdk/pluginmanager.cpp).
9. It installs no-op debugger window and menu factories inside the adapter, attaches the matched `cbDebuggerPlugin` through the public debugger plugin boundary [5](https://svn.code.sf.net/p/codeblocks/code/trunk/src/include/cbplugin.h), and exposes only debug launch, continue, break, stop, and official lifecycle events.
10. When configured, it loads a separately compiled DebuggerGDB data provider whose source revision, SDK tuple, compiler identity, language standard, build flags, and explicit ABI tag are checked before use [6](https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/debuggergdb/debugger_defs.h) [7](https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/debuggergdb/debuggerdriver.h).
