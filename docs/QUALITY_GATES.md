...
The three `real-toolchains-*.yml` workflows are deliberately isolated by operating system. Each runs on manual dispatch and weekly schedule, installs the versions recorded in the LSP/DAP matrices, verifies executable versions, and sets `CODIUM_BLOCKS_REQUIRE_REAL_LSP=1` and `CODIUM_BLOCKS_REQUIRE_REAL_DAP=1`. In strict mode, an unavailable required executable, an initialization failure, a known environment skip, or an incomplete scenario is a failure rather than an optional skip. The DAP matrix explicitly marks `OpenDebugAD7` as optional because the repository does not distribute that external Windows adapter; if it is installed, its scenario is still exercised and failures are reported. The ordinary platform workflows retain their conditional behavior so a developer machine without external servers remains usable. This separation keeps external tool availability from becoming a hidden dependency of the native build while still providing a periodic full-matrix signal.
