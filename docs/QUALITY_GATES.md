...
The three `real-toolchains-*.yml` workflows are deliberately isolated by operating system. Each runs on every push, manual dispatch, and a weekly schedule, installs the versions recorded in the LSP/DAP matrices, verifies executable versions, and sets `CODIUM_BLOCKS_REQUIRE_REAL_LSP=1` and `CODIUM_BLOCKS_REQUIRE_REAL_DAP=1`. In strict mode, an unavailable required executable, an initialization failure, or an incomplete scenario is a failure rather than an optional skip. The LSP matrix explicitly records the known Windows `gopls` workspace-view limitation as an environment-scoped optional case; it is still installed, version-checked, and exercised whenever the runner exposes a workspace view. The macOS DAP job uses the runner's Apple Clang with an explicit macOS SDK path to build the debuggee and selects the signed Xcode `debugserver` through `LLDB_DEBUGSERVER_PATH`, while the Windows job uses Clang CodeView/PDB output so LLDB-DAP receives native platform debug information, extracts the official LLVM 18.1.8 Windows installer with a checked-in SHA-256 through the runner's 7-Zip, and installs the exact Python 3.10.11 runtime required by the LLVM 18 Windows adapter, placing `python310.dll` beside `lldb-dap.exe` before validating adapter startup. The DAP session uses a bounded 30-second response window for cold debugger startup and LLDB-DAP's documented `debuggerRoot` launch field. For LLDB-DAP it sends `launch` and `configurationDone` together while the target is configured to stop at entry, then waits for the initial stop before sending source and function breakpoint requests. This accommodates LLDB-DAP's deferred launch-response behavior and avoids relying on pending pre-target breakpoints whose behavior differs across platform builds. The GDB path retains its established sequential launch/configuration flow. Source breakpoints remain part of the required scenario; function-breakpoint rejection is recorded as an adapter-specific unsupported capability when an adapter advertises the feature but does not accept the request. It records data-breakpoint discovery as an adapter-specific optional capability so a valid launch, stop, stack, scope, and variable scenario is not misclassified when an adapter advertises the feature but exposes no usable data identifier. The DAP matrix explicitly marks `OpenDebugAD7` as optional because the repository does not distribute that external Windows adapter; if it is installed, its scenario is still exercised and failures are reported. The ordinary platform workflows disable the external CTest registrations and rely on these isolated jobs for real execution. This separation keeps external tool availability from becoming a hidden dependency of the native build while still providing a full push and periodic matrix signal. On Windows, the portable CTest steps exclude the `conpty` and `fuzz` labels with `-LE "conpty|fuzz"` and fail promptly with the captured CTest output; the immediately following step runs the mandatory native ConPTY scenario. The bounded Extension Host fuzz remains a separately labeled test and the dedicated fuzz workflow retains its larger configured budget. A ConPTY startup problem or a process-heavy fuzz run therefore cannot be hidden inside an unrelated multi-test failure.
