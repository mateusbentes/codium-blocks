...
The next adapter increment is debugger data and selected-plugin policy refinement: serialize threads, stack frames, breakpoints, watches, variables, and source locations only after each field has a stable value-owned protocol mapping. Full Code::Blocks debugger/plugin compatibility is not implied. See [`docs/CODEBLOCKS_INTEGRATION.md`](CODEBLOCKS_INTEGRATION.md) and [`docs/CODEBLOCKS_ADAPTER_PROTOCOL.md`](CODEBLOCKS_ADAPTER_PROTOCOL.md).
