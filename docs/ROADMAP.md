...
The bridge remains discovery-only in the main process. Code::Blocks plugins depend on `Manager`, `PluginManager`, `cbPlugin`, event infrastructure, resources, and a matching SDK ABI; they must not be loaded as arbitrary shared libraries. Codium::Blocks imports `.cbp` targets and defines the versioned 1.0 event contract needed by an adapter.

The first production adapter increment is now implemented as an optional, separate `codium-blocks-codeblocks-adapter` executable. Its Phase A bootstrap owns a wxWidgets application and hidden frame, loads the official Code::Blocks resources, loads only an explicitly selected SDK-matched `Compiler` plugin, calls the real `ProjectManager::LoadProject()`, and emits `projectOpened` plus one structured `projectTarget` event per real `cbProject` target. It reports SDK identity, resources, compiler-plugin matching, and target-enumeration capabilities. Phase B now registers official SDK event sinks and normalizes project lifecycle/file events plus `cbEVT_COMPILER_STARTED` and `cbEVT_COMPILER_FINISHED` into value-owned JSON Lines events. Real compiler invocation, debugger, and arbitrary plugin loading are intentionally not claimed yet. Linux integration smokes verify both `.cbp` enumeration and event normalization when the SDK, resources, compiler plugin, and Xvfb are installed; the portable fake-adapter smoke test remains universal.

The next adapter increment is now Phase C: issue real batch compilation through the matched Code::Blocks Compiler plugin, capture compiler output and diagnostics, and normalize completion status. The adapter must only later consider debugger or selected plugin capabilities behind explicit ABI, manifest, provenance, and workspace-trust policies. See [`docs/CODEBLOCKS_INTEGRATION.md`](CODEBLOCKS_INTEGRATION.md) and [`docs/CODEBLOCKS_ADAPTER_PROTOCOL.md`](CODEBLOCKS_ADAPTER_PROTOCOL.md).
