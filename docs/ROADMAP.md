...
| Extension API depth | The Node host now supports workspace document open/change/save events, `workspace.textDocuments`, Tree Views, SCM providers, custom-editor provider activation, and a bounded native webview bridge that strips executable HTML and event handlers before publication. | This is a versioned native contract, not complete VS Code API parity: arbitrary browser APIs, unrestricted scripts, and renderer-specific behavior remain unsupported by design. |
| Extension registry and compatibility | The native registry parses bounded Open VSX results, caches catalogs, enforces HTTPS, same-origin and configured-registry policies, supports bearer authentication for private registries, verifies SHA-256 and optional Ed25519 signatures, installs transactionally, writes compatibility evidence, and exposes an explicit unattended-update policy. | Background updates remain disabled unless an operator supplies a registry policy and trusted key; the project does not silently trust arbitrary registries or unsigned artifacts. |
| Advanced DAP requests | The native client serializes function, data, conditional, hit-count, and logpoint requests; persists configured entries, parses initialize capabilities, requests adapter-specific `dataBreakpointInfo`, retains rejection messages, and exercises the fake and real protocol scenarios when capabilities permit. | External adapter version drift and broader LLDB/OpenDebugAD7 behavior remain real-toolchain validation concerns. |
...
The native client now has an adapter-specific discovery request and explicit capability report. The implementation continues to gate each request on the adapter's initialize capabilities rather than assuming that one adapter's extensions apply to all others. External adapter fixtures remain runner-owned because Codium::Blocks does not redistribute GDB, LLDB, or OpenDebugAD7.
...
The host now transports document lifecycle events, Tree View data, SCM resources, custom-editor activation, and sanitized webview state without exposing native pointers or private Code::Blocks objects. Each contribution has a deterministic smoke scenario and an explicit unsupported result when the native workbench cannot render it.

Webviews remain bounded and optional. The host does not execute extension-provided scripts, and the native terminal records Sixel and Kitty payload metadata under a strict size limit rather than pretending to render every image protocol.
...
Post-1.0 work may add mathematically complete Unicode grapheme and width handling, actual Sixel or Kitty raster rendering, unrestricted browser-compatible webviews, deeper VS Code API parity, and broader debugger features. The current patch deliberately implements bounded native contracts, strict policies, and deterministic evidence without making those unsupported claims.
