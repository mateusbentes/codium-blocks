...
The versioned matrix is now a required contract check. Dedicated platform workflows install or cache the pinned clangd, rust-analyzer, gopls, and pyright versions. The runtime harness creates isolated, language-appropriate workspaces and verifies initialize, document open, hover, completion, definition, declaration, references, document symbols, workspace symbols, and rename when the server advertises each provider. Because gopls creates its workspace view asynchronously, the harness gives it a short startup grace period and allows a bounded retry for the explicit `no views` startup response on slower runners. Missing executables or failures before protocol startup are reported as environment skips. Version mismatches and failures after the bounded startup window remain failures.
