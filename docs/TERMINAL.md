...
Sixel DCS and Kitty graphics APC payloads are consumed and discarded by policy rather than written into the visible screen. This prevents binary image payloads from corrupting terminal text. Rendering those protocols remains optional future work and would require explicit resource limits, image lifetime management, and a security policy.
...
This remains a focused native VT implementation rather than a complete xterm emulator. Future work is limited to complete grapheme segmentation, full Unicode width edge cases, synchronized-update edge cases, richer hyperlink behavior, and an optional, resource-limited image-rendering policy.
