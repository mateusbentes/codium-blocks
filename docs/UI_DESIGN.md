...
Codium::Blocks 1.0 will preserve the compact, direct character of the classic Code::Blocks interface [3](https://www.codeblocks.org/) while adding the practical feedback and navigation expected from a modern native IDE. The result should feel familiar to C and C++ developers, remain useful for other languages, and avoid the visual and resource cost of an Electron workbench.
...
The Debug panel uses the same visual language as Problems and Build. Breakpoints appear in the editor gutter and in a compact breakpoint list. The current native implementation persists requested source, function, and data breakpoints per workspace, sends conditional breakpoint, hit-condition, and logpoint fields, distinguishes pending, verified, and rejected states, and correlates adapter responses with the requested source file. Capability-gated function and data breakpoint requests are restored as pending at the start of each adapter session. A disabled state is reserved for a future adapter capability. The call stack is selectable, and selecting a frame opens the mapped source file at the reported line and column.
...
The interface must be keyboard-first without requiring the user to memorize hidden commands. Common actions must have visible shortcuts in menus and tooltips, consistent with the navigation patterns documented for modern code editors [2](https://code.visualstudio.com/docs/editor/editingevolved). The first 1.0 navigation set is:
...
The native editor deliveries implement the Find, Find next, Find previous, Replace, Replace all, and Go to Line actions through wxWidgets controls, together with conservative lexical syntax highlighting for common constructs in the detected language. When a language server advertises semantic tokens, the client reads its legend, requests full-document tokens, decodes delta positions, and overlays the result on the lexical styles. HTML `script` and `style` blocks receive bounded JavaScript and CSS fallback tokenization. The editor also offers Go to File, document/workspace symbol result navigation, definition/declaration/references requests, completion text-edit application, multi-block hover presentation, rename and code-action application, delimiter matching, automatic paired delimiters, basic indentation, and circular tab navigation. The search model also has case-sensitive and case-insensitive operations for future search-option controls. Highlighting remains intentionally layered rather than a universal parser; embedded grammars beyond the supported HTML regions, complete symbol indexing, richer replacement options, and advanced navigation remain future work. Versioned real-server manifests now exist; runtime execution remains conditional on installed tools.

## Extension and native contribution boundaries

Native C++ modules remain responsible for the editor, project model, compiler integration, terminal, debugger, SCM, and operating-system integration. The Node.js Extension Host remains responsible for JavaScript and TypeScript extensions. The current versioned host contracts cover commands, configuration, languages, workspace document events, and Tree View data. SCM providers and custom-editor descriptors remain native registries with bounded contribution seams rather than complete JavaScript implementations.
