...
The Debug panel should use the same visual language as Problems and Build. Breakpoints appear in the editor gutter and in a compact breakpoint list. The current native implementation persists requested breakpoints, sends conditional breakpoint, hit-condition, and logpoint fields, distinguishes pending, verified, and rejected states, and correlates adapter responses with the requested source file. A disabled state is reserved for a future adapter capability. The call stack is selectable, and selecting a frame opens the mapped source file at the reported line and column.
