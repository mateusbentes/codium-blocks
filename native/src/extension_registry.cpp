...
bool ExtensionRegistry::SearchOpenVsx(const wxString& registry, const wxString& query, wxString* json, wxString* error) const
{
    ...
    wxString response;
    if (!FetchText(url, &response, error)) {
...
{
    if (!ExtensionSecurity::IsAllowedRegistryUrl(registry) || !IsSafeSegment(namespaceName) ||
    ...
    wxString response;
    if (!FetchText(url, &response, error)) return false;
...
{
    if (!ExtensionSecurity::IsAllowedRegistryUrl(registry) || !IsSafeSegment(entry.namespaceName) ||
    ...
    output.Close();
    if (!output.IsOk() || total == 0 || !wxRenameFile(temporary, destination, true)) {
        wxRemoveFile(temporary);
    ...
    if (expectedSha256.empty() && !entry.sha256Url.empty()) {
        ...
        if (!FetchText(entry.sha256Url, &checksumText, error) || !ExtractSha256(checksumText, &expectedSha256)) {
            wxRemoveFile(destination);
