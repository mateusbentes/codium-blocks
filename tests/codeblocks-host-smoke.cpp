...
int main()
{
    if (codium::CodeBlocksHostContract::Version() != wxS("1.3") ||
        !codium::CodeBlocksHostContract::Supports(1, 0) ||
        !codium::CodeBlocksHostContract::Supports(1, 1) ||
        !codium::CodeBlocksHostContract::Supports(1, 2) ||
        !codium::CodeBlocksHostContract::Supports(1, 3) ||
    ...
    bus.Publish(codium::CodeBlocksHostEvent{
        ...
        wxEmptyString, 0, 0, 0, false, wxEmptyString, wxEmptyString, wxEmptyString, wxEmptyString});
    bus.Publish(codium::CodeBlocksHostEvent{
        ...
        wxS("bin/demo"), wxS("bin"), 0, 0, 0, false, wxEmptyString, wxEmptyString, wxEmptyString, wxEmptyString});
    bus.Publish(codium::CodeBlocksHostEvent{
        ...
        wxEmptyString, 11, 4, 1, true, wxEmptyString, wxEmptyString, wxEmptyString, wxEmptyString});
    ...
    bus.Publish(renamed);
    codium::CodeBlocksHostEvent snapshot;
    snapshot.kind = codium::CodeBlocksEventKind::DebugSnapshot;
    snapshot.dataKind = wxS("state");
    snapshot.snapshotJson = wxS("{\"stopped\":true}");
    ...
    if (events.size() != 5 || !bus.Empty() ||
        ...
        CodeBlocksEventKindName(events[4].kind) != wxS("debugSnapshot") ||
        events[4].dataKind != wxS("state") || events[4].snapshotJson.Find(wxS("stopped")) == wxNOT_FOUND) {
